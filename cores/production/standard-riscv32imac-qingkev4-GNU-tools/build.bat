...
for %%f in (*.c) do (
	echo %%f
	%GCC_PREFIX%gcc -pedantic -std=c99 -O2 -ffunction-sections -Wall -include includes.inc -march=rv32imac -mabi=ilp32 -I. -c -o %%~nf.o %%f
	call set OBJS=%%OBJS%% %%~nf.o
)

for %%f in (*.S) do (
	echo %%f
	%GCC_PREFIX%gcc -march=rv32imac -mabi=ilp32 -I. -include includes.inc -c -o %%~nf.o %%f
	call set OBJS=%%OBJS%% %%~nf.o
)
